...
sub get_command {
	...
	my $python = $config->python_cmd;

	# Warn if the Python interpreter is not executable:
	if ( defined($python) and ( $python ne '' ) and ( !-x $python ) ) {
		my $ret = Wx::MessageBox(
			Wx::gettext(
				sprintf( '%s seems to be no executable Python interpreter, use the system default Python instead?', $php )
			),
			Wx::gettext('Run'),
